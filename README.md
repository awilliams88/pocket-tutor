...
- **Demo**: [Video](Demo.mp4)
- **Social Post**: [LinkedIn](https://www.linkedin.com/posts/awilliams1988_buildsmall-huggingface-ai-ugcPost-7470803652820025346-SFt0)
...
- Hardware: Modal A10G
- Dataset: 16 structured tutoring examples plus 7 schema-aligned follow-up coaching examples
- Output format: the seven current production UI sections, not an older seed format
- Coverage: math, science, reading, writing, statistics, blurry image recovery, parent notes, and cheat-refusal examples
- Validation: Modal smoke test passes across all structured and follow-up examples
...
The Modal smoke test loads the production base model and LoRA adapter on Modal and checks every training and follow-up example against the seven-section UI contract. To smoke test only one prompt, run:

```bash
SMOKE_ALL_EXAMPLES=0 modal run modal/smoke.py
```

The Space smoke test calls the deployed Gradio API endpoint and verifies that all seven user-facing sections are generated by the model instead of falling back to default placeholder text.
...
| `modal/smoke.py` | Modal smoke test for the fine-tuned adapter and section contract |
| `modal/CARD.md` | Hugging Face model card for the LoRA adapter |

### `scripts/` - Deployment checks
| File | Purpose |
|---|---|
| `scripts/smoke_space.py` | Live Space API smoke test |
...
The Modal dataset includes math, science, reading, writing, statistics, blurry image recovery, parent-note coaching, schema-aligned follow-up turns, and explicit cheating refusal examples. The training target is the current production UI format, not an earlier seed format.
